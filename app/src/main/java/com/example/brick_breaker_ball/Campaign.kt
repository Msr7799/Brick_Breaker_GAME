...
object LevelRepository {
    ...
    val worlds = listOf(
        WorldDefinition(1, "CRYSTAL DEPTHS", "Crystal spire beneath the sea", "world_2_crystal", "6AA8FF"),
        WorldDefinition(2, "WINGED GUARDIANS", "Twin guardians of the shadow stairs", "world_2_crystal", "68C7E5"),
        WorldDefinition(3, "MOONLIT JUNGLE", "Moonlit palms and ancient ruins", "world_4_zerog", "8BA7FF"),
        WorldDefinition(4, "DESERT TEMPLE", "Sunlit temple in golden sands", "world_1_foundry", "E0A05C"),
        WorldDefinition(5, "GOLDEN HORIZON", "A lone ship on a golden horizon", "world_2_crystal", "72C5C8"),
        WorldDefinition(6, "ASTRAL GATE", "Blue nebula beyond the gate", "world_4_zerog", "6D8CFF"),
        WorldDefinition(7, "AZURE COAST", "Crystal beacon on the coast", "world_2_crystal", "4CB9F2"),
        WorldDefinition(8, "FROST CITADEL", "Ice palace beneath the aurora", "world_3_magma", "A5C9EA"),
        WorldDefinition(9, "VOID VORTEX", "A dark vortex in the void", "world_4_zerog", "7B5A8D"),
        WorldDefinition(10, "COSMIC WHALE", "A whale swims among the stars", "world_4_zerog", "7F9DBD"),
        WorldDefinition(11, "EMBER PORTAL", "Fiery portal in an ancient chamber", "world_3_magma", "A56B9C"),
        WorldDefinition(12, "CLOCKWORK CITADEL", "A clock at the end of time", "world_2_crystal", "8096AD"),
        WorldDefinition(13, "CRIMSON ECLIPSE", "Red eclipse above storm clouds", "world_3_magma", "D0784D")
    )
    ...
    private val worldDifficulty = mapOf(
        1 to WorldDifficulty(22, 2, 600f, 4.0f, .220f),
        2 to WorldDifficulty(25, 2, 620f, 4.0f, .214f),
        3 to WorldDifficulty(28, 2, 640f, 4.2f, .208f),
        4 to WorldDifficulty(31, 2, 660f, 4.2f, .202f),
        5 to WorldDifficulty(34, 2, 680f, 4.4f, .196f),
        6 to WorldDifficulty(37, 2, 700f, 4.4f, .190f),
        7 to WorldDifficulty(40, 2, 720f, 4.6f, .184f),
        8 to WorldDifficulty(43, 2, 740f, 4.6f, .178f),
        9 to WorldDifficulty(46, 2, 760f, 4.8f, .172f),
        10 to WorldDifficulty(49, 2, 780f, 4.8f, .166f),
        11 to WorldDifficulty(52, 2, 800f, 5.0f, .160f),
        12 to WorldDifficulty(54, 2, 820f, 5.0f, .155f),
        13 to WorldDifficulty(60, 2, 840f, 5.0f, .150f)
    )
    ...
    private fun createLevel(id: Int): LevelDefinition {
        ...
        positions.forEach { (row, column) ->
            place(row, column, materials[boardHash(id + 997, row, column) % materials.size])
        }

        if (hasExtraBrickRow) {
            val extraRow = rows - 1
            (0 until columns).forEach { column ->
                place(extraRow, column, materials[boardHash(id + 997, extraRow, column) % materials.size])
            }
        }
        ...
        // Keep a guaranteed carrier in every stage, but vary its position with the board.
        if (!bonusStage) {
            val carrierSpot = (0 until rows - 1).flatMap { row ->
                (0 until columns).map { column -> row to column }
            }.filter { (row, column) ->
                val type = cells[row][column]
                (type != null && (type in materials || type == BrickType.NORMAL_ONE_HIT)) &&
                    (stage != stageCount || row !in 2..4 || column !in 3..5)
            }.minByOrNull { (row, column) -> boardHash(id + 1997, row, column) }
            carrierSpot?.let { (row, column) -> place(row, column, BrickType.POWERUP_CARRIER) }
        }
        ...
        return LevelDefinition(
            ...
            dropRate = if (bonusStage) (difficulty.dropRate + .10f).coerceAtMost(.34f) else difficulty.dropRate,
    ...
    private fun boardHash(levelId: Int, row: Int, column: Int): Int {
        var hash = levelId * 0x9E3779B9u.toInt()
        hash = hash xor (row * 0x85EBCA6Bu.toInt()) xor (column * 0xC2B2AE35u.toInt())
        hash = (hash xor (hash ushr 16)) * 0x7FEB352D
        hash = (hash xor (hash ushr 15)) * 0x846CA68Bu.toInt()
...
class ProgressStore(private val prefs: Preferences = Gdx.app.getPreferences("brickbreakerball-progress-v1")) {
    ...
    var unlockedLevel: Int = prefs.getInteger("unlocked", LevelRepository.lastLevel(1)).coerceIn(1, LevelRepository.TOTAL_LEVELS)
        private set
    var hasStartedGame: Boolean = prefs.getBoolean("has_started_game", false)
        private set

    val highestUnlockedWorld: Int
        get() = computeAccessibleWorld()
    ...
    fun isWorldUnlocked(world: Int): Boolean = world in 1..highestUnlockedWorld

    fun isLevelUnlocked(level: Int): Boolean = isWorldUnlocked(LevelRepository.worldForLevel(level))

    fun nextRecommendedLevel(): Int {
        val world = highestUnlockedWorld
        val first = LevelRepository.firstLevel(world)
        val last = LevelRepository.lastLevel(world)
        return (first..last).firstOrNull { stars(it) == 0 } ?: last
    }

    private fun computeAccessibleWorld(): Int {
        var accessibleWorld = 1
        for (world in 1 until LevelRepository.worlds.size) {
            val first = LevelRepository.firstLevel(world)
            val last = LevelRepository.lastLevel(world)
            val currentWorldComplete = (first..last).all { stars(it) > 0 }
            if (!currentWorldComplete) break
            accessibleWorld = world + 1
        }
        return accessibleWorld.coerceIn(1, LevelRepository.worlds.last().id)
    }

    private fun syncUnlockProgress() {
        val accessibleWorld = computeAccessibleWorld()
        unlockedLevel = LevelRepository.lastLevel(accessibleWorld)
        prefs.putInteger("unlocked", unlockedLevel).flush()
    }
    ...
    fun complete(level: Int, score: Int, stars: Int) {
        prefs.putInteger("stars_$level", maxOf(this.stars(level), stars))
