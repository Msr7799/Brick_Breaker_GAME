/*
 * ملاحظات صيانة الملف:
 * المسار: app/src/main/java/com/example/brick_breaker_ball/PausedSessionStore.kt
 * المؤلف: mohamed alromaihi
 * الدوال الموجودة: `hasPausedGame`، `save`، `restore`، `clear`
 */
...
class PausedSessionStore(private val prefs: Preferences = Gdx.app.getPreferences("brickbreakerball-paused-session-v1")) {
    /** ملاحظة صيانة: الدالة `hasPausedGame` تتحقق من الشرط المطلوب وتعيد نتيجة يمكن لبقية النظام الاعتماد عليها؛ راجع استدعاءاتها واختباراتها قبل تعديلها. */
    ...
    fun restore(): Pair<LevelDefinition, GameSession>? {
        ...
        return runCatching {
            ...
            session.paddle.x = prefs.getFloat("paddle", 450f)
            session.paddle.width = prefs.getFloat("paddleWidth", 244f)
            session.paddle.targetWidth = prefs.getFloat("targetWidth", session.paddle.width)
            session.paddle.mode = PaddleMode.valueOf(prefs.getString("paddleMode", PaddleMode.NORMAL.name))
            session.balls.clear()
            prefs.getString("balls", "").split(';').filter(String::isNotBlank).forEach { raw ->
                ...
                session.balls += if (version >= 7 && v.size >= 15) {
                    ...
                    val legacyMode = v[8]
                    val size = BallSize.fromRadius(v[7].toFloat())
                    Ball(
                        id = v[0].toInt(), position = Vector2(v[1].toFloat(), v[2].toFloat()), previousPosition = Vector2(v[3].toFloat(), v[4].toFloat()), velocity = Vector2(v[5].toFloat(), v[6].toFloat()), size = size, baseSize = BallSize.DEFAULT,
                        element = if (legacyMode == "FIRE") {
                            BallElement.FIRE
                        } else if (legacyMode == "EXPLOSIVE") {
                            BallElement.EXPLOSIVE
                        } else {
                            BallElement.NORMAL
                        },
            ...
            prefs.getString("bricks", "").split(';').filter(String::isNotBlank).forEach { raw ->
                ...
                session.bricks += Brick(
                    v[0].toInt(), Rectangle(v[1].toFloat(), v[2].toFloat(), v[3].toFloat(), v[4].toFloat()), type, health, v[7].toFloat(), v[8].toFloat(), v[9].toFloat(), v[10].toFloat(),
            ...
            session.fallingPowerUps.clear()
            prefs.getString("powers", "").split(';').filter(String::isNotBlank).forEach { raw ->
                val v = raw.split(',')
                runCatching { PowerUpType.valueOf(v[1]) }.getOrNull()?.let { type -> session.fallingPowerUps += FallingPowerUp(v[0].toInt(), type, Vector2(v[2].toFloat(), v[3].toFloat()), Vector2(v[4].toFloat(), v[5].toFloat())) }
            }
            session.powerUps.clear()
            prefs.getString("timers", "").split(';').filter(String::isNotBlank).forEach { raw ->
                val v = raw.split(',')
                runCatching { PowerUpType.valueOf(v[0]) }.getOrNull()?.let { type ->
                    if (PowerUpCatalog.definitions.getValue(type).scope == EffectScope.TIMED) {
                        session.powerUps.timers[type] = v[1].toFloat()
                    } else {
                        session.powerUps.restorePersistent(listOf(type))
                    }
                }
            }
            ...
            session.baseBallSize = restoredBaseSize
            session.balls.forEach { ball ->
                ball.baseSize = restoredBaseSize
                ball.size = when {
                    PowerUpType.SHRINK_BALL in session.powerUps -> restoredBaseSize.smaller()
                    PowerUpType.MEGA_BALL in session.powerUps -> restoredBaseSize.megaBoosted()
                    else -> restoredBaseSize
                }
            }
            session.laserShots.clear()
            prefs.getString("lasers", "").split(';').filter(String::isNotBlank).forEach { raw ->
                val v = raw.split(',')
                session.laserShots += if (version >= 9 && v.size >= 6) {
                    LaserShot(
                        Vector2(v[0].toFloat(), v[1].toFloat()),
                        Vector2(v[2].toFloat(), v[3].toFloat()),
                        fire = v[4].toBoolean(),
                        piercing = v[5].toBoolean()
                    )
                } else {
                    LaserShot(Vector2(v[0].toFloat(), v[1].toFloat()), Vector2(v[2].toFloat(), v[3].toFloat()))
                }
            }
            session.laserCooldown = prefs.getFloat("laserCooldown", 0f)
            session.nextBallId = prefs.getInteger("nextBallId", (session.balls.maxOfOrNull { it.id } ?: 0) + 1)
            session.nextPowerUpId = prefs.getInteger("nextPowerUpId", 1)
            session.bottomShield = prefs.getBoolean("shield", false)
