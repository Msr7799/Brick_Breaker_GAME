...
class CampaignProgressionTest {
    @Test fun ninetyLevelsAreNineByEightUniqueAndDistributedAcrossThirteenWorlds() {
        ...
        assertTrue(LevelRepository.levels.all { level -> level.rows == 8 && level.columns == 9 && level.layout.size == 8 && level.layout.all { row -> row.length == 9 } })
        (1..13).forEach { world ->
            val layouts = LevelRepository.levels.filter { it.world == world }.map { it.brickIds.entries.sortedBy { entry -> entry.key }.joinToString() }
    ...
    @Test fun worldsIntroduceAllPermanentBrickTypesAndDifficultyRisesAcrossWorlds() {
        ...
        assertTrue(LevelRepository.levels.filter { it.world == 6 }.all { BrickType.KEY_BRICK.name in it.brickIds.values && BrickType.LOCKED.name in it.brickIds.values })
    ...
    @Test fun carrierAndDropRatesGiveEveryWorldRandomPowerUpOpportunities() {
        (1..13).forEach { world ->
            ...
            assertTrue(levels.any { BrickType.POWERUP_CARRIER.name in it.brickIds.values || BrickType.RANDOM_INVENTORY_POWERUP.name in it.brickIds.values })
